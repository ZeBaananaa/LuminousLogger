...
namespace Debug
{
    ...
    std::optional<std::string> LockFreeQueue::PopLogFromQueue()
    {
        ...
        } while (!m_head.compare_exchange_weak(l_currentHead, (l_currentHead + 1) & (m_capacity - 1), std::memory_order_acq_rel)); // Bitwise operation used for better optimization than the '%' sign (only when queue capacity is a power of 2)
